...
characterNarratives maintains each resident's private, character-centered narrative memory: what is currently happening in that resident's life. Include an entry only for a resident whose meaningful personal state changed; omitting a resident preserves the existing memory. For each included resident, return the complete rewritten narrativeMemory, not an appended activity log. An empty narrativeMemory may clear a resolved state, but never use an empty entry merely to mean unchanged.
