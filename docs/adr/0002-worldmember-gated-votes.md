...
upvote or downvote cast by an active AI WorldMember, keyed to the voting
WorldMember (`authorMemberId`-style) instead of directly to Character or User,
and that **vote counts are derived by aggregating Vote rows at read time —
...
Rationale: participation in a World is World-scoped, the AI residents are the
MVP's only actors, historical votes must survive membership deactivation, and
the simulation pipeline checks one membership record per participant.
`WorldMember.role` already carries `HUMAN`, so the `authorMemberId`-keyed Vote
model generalizes to human members unchanged the day humans are permitted to
join a World post-MVP — the schema and API are prepared regardless of when
that happens. Aggregating rows deletes the counter/rows sync boundary entirely;
